...
export default function Project({ projectId }) {
	...
	const totalTaktziv = (project.totalTakzuvCoachAdam || 0) + (project.totalTakzivRechesh || 0);
		...
		<article className="project-card" dir="rtl" aria-label={project.name}>
			<header className="pc-header">
				<div className="pc-badges">
					{project.shortTag && <span className="pc-pill">{project.shortTag}</span>}
					{project.unit && <span className="pc-subpill">{project.unit}</span>}
				</div>
			...
			<section className="pc-grid">
				...
				<div className="pc-item">
					...
					<div className="pc-item-value pc-gap">{formatMoney(Math.abs(gap))}{gap > 0 ? <span className="pc-arrow">▲</span> : null}</div>
				</div>
			</section>
